...
class ScholarshipResponse implements Responsable
{
    ...
    {
        $fields = ['id', 'title', 'place', 'status', 'scholarship_type', 'deadline', 'image', 'description', 'user_id', 'created_at', 'updated_at'];
        $request->request->add(['fields' => $fields]);
        $text = $this->options['text'];
        $paginateLimit = $this->options['limit'];
        $data = Posts::select($fields)->where('type', 'scholarship');
        ...
        $data->map(function ($d) {
            ...
            $d->filename = $d->image;
            $d->scholarship_deadline = Helpers::toFormatDateString($d->deadline); //$d->start_date->format('c');//covert timestamp to ISO 8601 format.
    ...
    public function toResponse($request)
    {
        ...
        if (Helpers::isAjax($request)) {
            if ($this->actionType === 'insert') {
                ...
                $info->scholarship_type = $this->getScholarshipTypes($request->get('scholarship_type'));
                $info->deadline = Helpers::toFormatDateString($request->get('deadline'), 'Y-m-d H:i:s');
                $info->type = 'scholarship';
    ...
    public function getScholarshipTypes($title)
    {
        $types = ['full', 'partial'];
        if (in_array($title, $types, true)) {
            return $title;
        }
        return 'full';
    }
